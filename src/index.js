...
const main = async () => {
  ...
  while (currentPage < totalPages) {
    ...
    const {
      data: nextData,
      page: nextPage
    } = await getNextPages(nextPageUrl);
...
if (test) {
  ...
  const conversationURL = `https://api.intercom.io/conversations?starting_after=WzE3MDYwMTkzNzMwMDAsOTc1NDcxMTQyODA1MDUsMjRd`;

  (async () => {

    //const data = await  getConversationDetails(conversationURL);
